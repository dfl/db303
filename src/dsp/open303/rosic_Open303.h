...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    /** Enables modelling of the µPD650C CPU's interrupt-clock timing (see Schmidt, "Analysis of the
     *  µPD650C-133 CPU timing"). The 303 services a fixed ~1.8 ms interrupt that is asynchronous to
     *  the tempo clock, so the gate-off edge snaps to that grid and its position beats against the
     *  tempo - the step duty cycle wanders (~50-56%), most audible at fast tempos. Off by default. */
